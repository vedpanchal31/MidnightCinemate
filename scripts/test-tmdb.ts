...
async function testTMDB() {
  ...
  try {
    ...
    if (nowPlaying.results.length > 0) {
      ...
      console.log(`   Example: "${firstMovie.title}" (${firstMovie.release_date})`);
      console.log(`   Poster: ${tmdbService.getPosterUrl(firstMovie.poster_path)}`);
    }
    console.log();
    ...
    if (popular.results.length > 0) {
      ...
      console.log(`   Example: "${firstMovie.title}" (Rating: ${firstMovie.vote_average}/10)`);
    }
    console.log();
    ...
    if (error instanceof Error && error.message.includes('TMDB_API_KEY is not configured')) {
      console.log('\n📌 Next Steps:');
      ...
      console.log('   5. Add it to your .env file as TMDB_API_KEY=your_key_here');
    }
    process.exit(1);
  }
}
