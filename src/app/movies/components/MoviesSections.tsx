...
import { Movie, useGetHindiQuery, useGetNowPlayingQuery } from "@/store/moviesApi";
...
export default function MoviesSections() {
  const { data: nowPlayingData, isFetching: isNowPlayingLoading } = useGetNowPlayingQuery();
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-bold tracking-tight mb-2">Now Showing</h1>
      ...
      {isNowPlayingLoading ? (
        <p className="text-sm text-muted-foreground">Loading now playing movies…</p>
      ) : (
      ...
      <section className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Popular releases from Bollywood and beyond</p>
        </div>
        {isHindiLoading ? (
