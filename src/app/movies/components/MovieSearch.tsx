    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-3">
                <label htmlFor="movie-search" className="text-sm uppercase tracking-[0.3em] text-muted-foreground">
                    Search movies
                </label>
            ...
            {isFetching && (
                <p className="text-sm text-muted-foreground">Searching for “{trimmedQuery}”…</p>
            )}
            ...
            {!isFetching && trimmedQuery && movies.length === 0 && !isError && (
                <p className="text-sm text-muted-foreground">No matches found. Try another title.</p>
            )}
            ...
            {movies.length > 0 && (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {movies.map((movie) => (
                            <div key={movie.id} className="group relative rounded-xl overflow-hidden bg-white/5 border border-white/10 hover:border-primary/50 transition-colors">
                                <div className="aspect-2/3 relative">
                                    {movie.poster_url ? (
                                        <>
                                            ...
                                            />
                                            <div className="absolute top-2 left-2 bg-black/80 text-white text-xs px-2 py-1 rounded opacity-75">
                                                DEBUG: {movie.poster_url?.substring(0, 50)}...
                                            </div>
                                        </>
                                    ) : (
                                ...
                                <div className="p-4 space-y-2">
                                    <div className="flex justify-between items-start gap-2">
                                        <h3 className="font-semibold text-lg leading-tight truncate">{movie.title}</h3>
