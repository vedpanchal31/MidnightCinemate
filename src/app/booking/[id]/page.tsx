...
const SEAT_TYPES = {
    VIP: { label: 'VIP', price: 500, color: 'bg-purple-600', hover: 'hover:bg-purple-500' },
    PREMIUM: { label: 'Premium', price: 300, color: 'bg-blue-600', hover: 'hover:bg-blue-500' },
    STANDARD: { label: 'Standard', price: 150, color: 'bg-zinc-600', hover: 'hover:bg-zinc-500' },
};
...
const generateSeats = (): Seat[] => {
    ...
    rows.forEach((row, rowIndex) => {
        ...
        for (let i = 1; i <= 12; i++) {
            ...
            const isReserved = (row.charCodeAt(0) + i) % 6 === 2 || (row.charCodeAt(0) * i) % 11 === 4;
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col">
            ...
            <header className="p-4 border-b border-white/10 backdrop-blur-md sticky top-0 z-50 bg-black/50">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/movies" className="p-2 hover:bg-white/10 rounded-full transition-colors">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-zinc-400">{theaterName} | {showTime}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/10">
                            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                            <span className="text-xs font-medium text-zinc-300">Fast Booking Active</span>
                        </div>
                    </div>
                </div>
            </header>
                    ...
                    <div className="min-w-[600px] flex flex-col items-center gap-4">
                        {['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'].map((row, rowIndex) => (
                            <div key={row} className={cn(
                                ...
                                <div className="w-6 text-xs font-black text-zinc-700 select-none">{row}</div>
                                ...
                                <div className="flex gap-3">
                                    {INITIAL_SEATS.filter(s => s.row === row).map((seat) => {
                                        ...
                                        const marginClass = (seat.number === 4 || seat.number === 8) ? "mr-10" : "";
                                                    ...
                                                    isBooked
                                                        ...
                                                        : isSelected
                                                            ...
                                                            : cn(
                                                                ...
                                                                seat.type === 'VIP' ? "border-purple-500/30" : seat.type === 'PREMIUM' ? "border-blue-500/30" : ""
                                                            )
                                                )}
                                            >
                                                {/* Seat Number Tooltip */}
                                                <div className="absolute -top-10 left-1/2 -translate-x-1/2 px-2 py-1 bg-zinc-800 rounded text-[9px] font-bold opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border border-white/10 whitespace-nowrap z-50">
                                                    {seat.type} {row}{seat.number} • ₹{seat.price}
                                                </div>
            ...
            <div className={cn(
                ...
                selectedSeats.length > 0 ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0 pointer-events-none"
            )}>
                <div className="bg-zinc-900/80 backdrop-blur-2xl border border-white/10 p-5 rounded-[2rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.8)] flex flex-col md:flex-row items-center justify-between gap-6 px-10">
                    <div className="flex items-center gap-8">
                        <div className="space-y-1">
                            <p className="text-[10px] tracking-widest uppercase font-bold text-zinc-500">Tickets</p>
                            <div className="flex flex-wrap gap-2">
                                {selectedSeats.sort().map(s => (
                                    <span key={s} className="px-3 py-1 bg-primary/20 text-primary text-xs font-black rounded-lg border border-primary/30">
                                        {s}
                                    </span>
                                ))}
                            </div>
                        ...
                        <div className="space-y-0">
                            <p className="text-[10px] tracking-widest uppercase font-bold text-zinc-500">Payable Amount</p>
                            <div className="flex items-baseline gap-1">
                                <span className="text-3xl font-black text-white">₹{totalPrice}</span>
                                <span className="text-zinc-500 text-xs font-medium">incl. taxes</span>
                            </div>
                        </div>
                    </div>

                    <Button asChild className="w-full md:w-auto px-10 h-14 bg-primary hover:bg-primary/90 text-white font-black text-lg rounded-2xl shadow-[0_10px_30px_-10px_rgba(229,9,20,0.5)] group">
