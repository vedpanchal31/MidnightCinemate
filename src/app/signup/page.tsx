...
import { useGoogleLoginMutation, useSignUpMutation } from "@/store/authApi";
...
export default function SignUpPage() {
  ...
  const [googleLogin, { isLoading: isGoogleSigningIn }] =
  ...
  const handleGoogleSignUpClick = React.useCallback(() => {
    if (!isGoogleScriptLoaded) {
      showToast.error("Google signup is still loading. Please try again.");
      return;
    }

    const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID;
    const googleOauthApi = window.google?.accounts?.oauth2;
    ...
    if (!googleOauthApi) {
      showToast.error("Google login is not available right now.");
      return;
    }
  ...
  }, [dispatch, googleLogin, isGoogleScriptLoaded, router]);
  ...
  React.useEffect(() => {
    ...
    script.onerror = () => setIsGoogleScriptLoaded(false);
